...
namespace Project1
{
	internal static class EQMath
	{
		...
		internal static PierSide2 SOP_Pointing(double DEC)
		{
			...
			if (DEC <= 90 || DEC >= 270)
			{
				if (Common.gAscomCompatibility.SwapPointingSideOfPier)
				{
		...
		internal static eqmodvector.Coordt DeltaSync_Matrix_Map(double RA, double DEC)
		{
			eqmodvector.Coordt result = new eqmodvector.Coordt();
			int i = 0;
			...
			if (i != -1)
			{
				...
				result.x = RA + (Alignment.ct_Points[i - 1].x - Alignment.my_Points[i - 1].x) + gRASync01;
				result.Y = DEC + (Alignment.ct_Points[i - 1].Y - Alignment.my_Points[i - 1].Y) + gDECSync01;
				result.z = 1;
				result.f = 0;
			}
			else
