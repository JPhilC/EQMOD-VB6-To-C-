...
namespace Project1
{
	internal static class eqmodvector
	{
		...
		// obj contains encoder positions. Not angles.
		internal static Coord EQ_sp2Cs(Coord obj)
		{
			...
			{
				tmpobj4 = EQ_SphericalPolar(obj.x, obj.Y, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos, EQMath.gLatitude);
				tmpobj = EQ_Polar2Cartes(tmpobj4.x, tmpobj4.Y, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos);
				result.x = tmpobj.x;
				result.Y = tmpobj.Y;
				result.z = 1;
			}
		...
		internal static Coord EQ_sp2Cs2(Coord obj)
		{
			...
			if (Convert.ToDouble(HC.PolarEnable.Value) == 1)
			{
				tmpobj4 = EQ_SphericalPolar(obj.x, obj.Y, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos, Math.Abs(EQMath.gLatitude));
				tmpobj = EQ_Polar2Cartes(tmpobj4.x, tmpobj4.Y, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos);
				result.x = tmpobj.x;
				result.Y = tmpobj.Y;
				result.z = 1;
		...
		internal static Coord EQ_pl2Cs(Coord obj)
		{
			...
			if (Convert.ToDouble(HC.PolarEnable.Value) == 1)
			{
				tmpobj = EQ_Polar2Cartes(obj.x, obj.Y, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos);
		...
		internal static Coord EQ_plAffine(Coord obj)
		{
			...
			if (Convert.ToDouble(HC.PolarEnable.Value) == 1)
			{
				...
				tmpobj1 = EQ_Polar2Cartes(tmpobj4.x, tmpobj4.Y, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos);
				...
				tmpobj2 = EQ_Cartes2Polar(tmpobj3.x, tmpobj3.Y, tmpobj1.r, tmpobj1.RA, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos);

				result = EQ_PolarSpherical(tmpobj2.x, tmpobj2.Y, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos, EQMath.gLatitude, tmpobj4.r);
		...
		internal static CartesCoord EQ_Polar2Cartes(double RA, double DEC, double TOT, double RACENTER, double DECCENTER)
		{
			...
			double theta = EQMath.Range360(i) * EQMath.DEG_RAD;
		...
		internal static Coord EQ_Cartes2Polar(double x, double Y, double r, double RA, double TOT, double RACENTER, double DECCENTER)
		{
			...
			if (r < 0)
			{
				angle = EQMath.Range360(angle + 180);
			}
		...
		internal static int EQ_UpdateTaki(double x, double Y)
		{
			...
			switch(Common.g3PointAlgorithm)
			{
				case 1 : 
			...
			if (EQMath.gTaki1 == 0 || EQMath.gTaki2 == 0 || EQMath.gTaki3 == 0)
			{
				return 0;
			}
			...
			return EQ_AssembleMatrix_Taki(tmpcoord.x, tmpcoord.Y, Alignment.ct_PointsC[Convert.ToInt32(EQMath.gTaki1) - 1], Alignment.ct_PointsC[Convert.ToInt32(EQMath.gTaki2) - 1], Alignment.ct_PointsC[Convert.ToInt32(EQMath.gTaki3) - 1], Alignment.my_PointsC[Convert.ToInt32(EQMath.gTaki1) - 1], Alignment.my_PointsC[Convert.ToInt32(EQMath.gTaki2) - 1], Alignment.my_PointsC[Convert.ToInt32(EQMath.gTaki3) - 1]);
		...
		internal static int EQ_UpdateAffine(double x, double Y)
		{
			...
			EQMath.gAffine1 = tr.i;
			EQMath.gAffine2 = tr.j;
			EQMath.gAffine3 = tr.k;

			if (EQMath.gAffine1 == 0 || EQMath.gAffine1 == 0 || EQMath.gAffine1 == 0)
			{
				return 0;
			}
			...
			result = EQ_AssembleMatrix_Affine(tmpcoord.x, tmpcoord.Y, Alignment.my_PointsC[Convert.ToInt32(EQMath.gAffine1) - 1], Alignment.my_PointsC[Convert.ToInt32(EQMath.gAffine2) - 1], Alignment.my_PointsC[Convert.ToInt32(EQMath.gAffine3) - 1], Alignment.ct_PointsC[Convert.ToInt32(EQMath.gAffine1) - 1], Alignment.ct_PointsC[Convert.ToInt32(EQMath.gAffine2) - 1], Alignment.ct_PointsC[Convert.ToInt32(EQMath.gAffine3) - 1]);
		...
		internal static void EQ_Quicksort(double[] List, double[] Sublist, int min, int max)
		{
			...
			int i = Convert.ToInt32(Math.Floor((double) ((max - min + 1) * VBMath.Rnd() + min)));
		...
		internal static SphereCoord EQ_SphericalPolar(double RA, double DEC, double TOT, double RACENTER, double DECCENTER, double Latitude)
		{
			...
			double i = EQMath.Get_EncoderHours(RACENTER, RA, TOT, 0);
			double j = EQMath.Get_EncoderDegrees(DECCENTER, DEC, TOT, 0) + 270;
			j = EQMath.Range360(j);

			// Convert encoder RA hours and dec degrees into Alt-Az radians.
			object tempAuxVar = hadec_aa[Convert.ToInt32(Latitude * EQMath.DEG_RAD), Convert.ToInt32(i * EQMath.HRS_RAD), Convert.ToInt32(j * EQMath.DEG_RAD), Convert.ToInt32(Y), Convert.ToInt32(x)];

			result.x = ((((x * EQMath.RAD_DEG) - 180) / 360d) * TOT) + RACENTER;
			result.Y = ((((Y * EQMath.RAD_DEG) + 90) / 180d) * TOT) + DECCENTER;
		...
		internal static Coord EQ_PolarSpherical(double RA, double DEC, double TOT, double RACENTER, double DECCENTER, double Latitude, double range)
		{
			...
			object tempAuxVar = aa_hadec[Convert.ToInt32(Latitude * EQMath.DEG_RAD), Convert.ToInt32(j * EQMath.DEG_RAD), Convert.ToInt32(i * EQMath.DEG_RAD), Convert.ToInt32(x), Convert.ToInt32(Y)];

			if (i > 180)
			{
				if (range == 0)
				{
					Y = EQMath.Range360(180 - (Y * EQMath.RAD_DEG));
				}
				else
				{
					Y = EQMath.Range360(Y * EQMath.RAD_DEG);
				}
			}
			else
			{
			...
			j = EQMath.Range360(Y + 90);

			if (j < 180)
			{
				if (range == 1)
				{
					x = EQMath.Range24(x * EQMath.RAD_HRS);
				}
				else
				{
					x = EQMath.Range24(24 + (x * EQMath.RAD_HRS));
				}
			}
			else
			{
				x = EQMath.Range24(12 + (x * EQMath.RAD_HRS));
			}


			result.x = EQMath.Get_EncoderfromHours(RACENTER, ref x, TOT, 0);
			double tempRefParam = Y + 90;
			result.Y = EQMath.Get_EncoderfromDegrees(DECCENTER, ref tempRefParam, TOT, 0, 0);
		...
		internal static CartesCoord EQ_Spherical2Cartes(double RA, double DEC, double TOT, double RACENTER, double DECCENTER)
		{
			...
			CartesCoord result = new CartesCoord();
			SphereCoord tmpobj4 = EQ_SphericalPolar(RA, DEC, TOT, RACENTER, DECCENTER, EQMath.gLatitude);
		...
		internal static Coord EQ_Cartes2Spherical(double x, double Y, double r, double RA, double range, double TOT, double RACENTER, double DECCENTER)
		{
			...
			return EQ_PolarSpherical(tmpobj2.x, tmpobj2.Y, EQMath.gTot_step, EQMath.RAEncoder_Home_pos, EQMath.gDECEncoder_Home_pos, EQMath.gLatitude, range);
		...
		internal static TriangleCoord EQ_Choose_3Points(double x, double Y)
		{
			...
			Tdatholder[] datholder = ArraysHelper.InitializeArray<Tdatholder>(Alignment.MAX_STARS);
			double combi_cnt = 0;
			double tmp1 = 0;
			int tmp2 = 0;
			bool first = false;
			...
			for (i = 1; i <= tempForEndVar; i++)
			{
				...
				datholder[Count].cc = Alignment.my_PointsC[Convert.ToInt32(i) - 1];
				switch(Common.gPointFilter)
				{
					...
					case 2 : 
						// local quadrant 
						if (EQMath.GetQuadrant(tmpcoord) != EQMath.GetQuadrant(datholder[Count].cc))
						{
							goto NextPoint;
						} 
				...
				if (Convert.ToDouble(HC.CheckLocalPier.Value) == 1)
				{
					...
					datholder[Count].dat = Math.Pow(Alignment.my_Points[Convert.ToInt32(i) - 1].x - x, 2) + Math.Pow(Alignment.my_Points[Convert.ToInt32(i) - 1].Y - Y, 2);
				}
				else
				{
			...
			if (Count > Alignment.gMaxCombinationCount - 1)
			{
				combi_cnt = Alignment.gMaxCombinationCount;
			}
			else
			{
				combi_cnt = Count;
