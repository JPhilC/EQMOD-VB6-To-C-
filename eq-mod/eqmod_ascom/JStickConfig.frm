...
Private Sub CheckPOV_Click()
    ...
    Else
        JoystickCal.POVEnabled = 0
    End If
    SaveJoystickCalib
End Sub

Private Sub CheckSwapXY_Click()
    If CheckSwapXY.Value = 1 Then
        JoystickCal.SwapXY = 1
    Else
...
'    While i = JOYERR_NOERROR
    While j < 10
        ...
        Else
            ' try some thing else!
            i = joyGetPosEx(j, JoystickDat)
            If i = JOYERR_NOERROR Then
                Combo1.AddItem (CStr(j) & ": Unnamed Gamepad")
            End If
        End If
        j = j + 1
    Wend
...
'        i = joyGetDevCaps(JOYSTICKcal.ID, JoystickInfo, Len(JoystickInfo))
    ...
    If JoystickCal.StickEnabled = 0 Then
        Check2.Value = 0
    Else
        Check2.Value = 1
    End If
    ...
    If JoystickCal.POVEnabled = 1 Then
        CheckPOV.Value = 1
    Else
        CheckPOV.Value = 0
    End If
...
Private Sub StartBtn_Click()
    ...
    If StartBtn.Caption = oLangDll.GetLangString(1209) Then
        ...
        JoystickCal.dwMaxXpos = 0
        JoystickCal.dwMinYpos = 32767
        JoystickCal.dwMaxYpos = 0
        JoystickCal.dwMinZpos = 32767
        JoystickCal.dwMaxZpos = 0
        JoystickCal.dwMinRpos = 32767
        JoystickCal.dwMaxRpos = 0
        Timer2.Enabled = True
        StartBtn.Caption = oLangDll.GetLangString(1208)
    Else
        Timer2.Enabled = False
        If (JoystickCal.dwMaxXpos - JoystickCal.dwMinXpos) > 32768 Then
            mid = 32767
        Else
            mid = 16383
        End If
        ...
        quarter = (mid - JoystickCal.dwMinXpos) / 4
        JoystickCal.dwX25Left = mid - quarter
        JoystickCal.dwX75left = mid - (quarter * 3)
        JoystickCal.dwX90left = mid - (mid - JoystickCal.dwMinXpos) * 9 / 10
        
        quarter = (JoystickCal.dwMaxXpos - mid) / 4
        JoystickCal.dwX25Right = mid + quarter
        JoystickCal.dwX75Right = mid + (quarter * 3)
        JoystickCal.dwX90Right = mid + (JoystickCal.dwMaxXpos - mid) * 9 / 10
        
        If (JoystickCal.dwMaxYpos - JoystickCal.dwMinYpos) > 32768 Then
            mid = 32767
        Else
            mid = 16383
        End If
        
        quarter = (mid - JoystickCal.dwMinYpos) / 4
        JoystickCal.dwY25Left = mid - quarter
        JoystickCal.dwY75left = mid - (quarter * 3)
        JoystickCal.dwY90left = mid - (mid - JoystickCal.dwMinYpos) * 9 / 10
        
        quarter = (JoystickCal.dwMaxYpos - mid) / 4
        JoystickCal.dwY25Right = mid + quarter
        JoystickCal.dwY75Right = mid + (quarter * 3)
        JoystickCal.dwY90Right = mid + (JoystickCal.dwMaxYpos - mid) * 9 / 10
