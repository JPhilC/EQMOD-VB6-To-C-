...
Begin VB.Form polarfrm 
   ...
   Begin VB.Frame CmdFrame 
      ...
      Top             =   360
      Width           =   1695
      Begin VB.CheckBox CheckHomeEnable 
         BackColor       =   &H00000000&
         Caption         =   "Disabled"
         BeginProperty Font 
            Name            =   "Arial Narrow"
...
Private Sub CheckHomeEnable_Click()
    If CheckHomeEnable.value = 1 Then
        SetHomeCmd.Enabled = False
    Else
        SetHomeCmd.Enabled = True
    End If
    Call HC.oPersist.WriteIniValue("POLAR_HOME_DISABLE", CStr(CheckHomeEnable.value))
    ...
    If gEQparkstatus = 0 Then
        If HC.ChkEnableLimits.value = 0 Then
            ...
            If gRAEncoderPolarHomeGoto <> 0 And gDECEncoderPolarHomeGoto <> 0 Then
                ' polar home is defined so allow user to move to it
                MoveHomeCmd.Enabled = True
                tmptxt = HC.oPersist.ReadIniValue("POLAR_HOME_DISABLE")
                If tmptxt = "0" Then
                    ' allow setting of new home position
                    CheckHomeEnable.value = 0
                    SetHomeCmd.Enabled = True
                Else
                    ' prevent setting of new home position
                    CheckHomeEnable.value = 1
                    SetHomeCmd.Enabled = False
                End If
            Else
                ' home position hasn't been defined yet
                MoveHomeCmd.Enabled = False
                CheckHomeEnable.value = 0
            End If
            status = 0
...
Private Sub SetText()
    ...
    CheckHomeEnable.Caption = oLangDll.GetLangString(6018)
End Sub
